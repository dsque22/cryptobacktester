...
import matplotlib.pyplot as plt
import seaborn as sns
sns.set_style("darkgrid")  # More reliable approach
...
def create_performance_report(results, strategy_name: str = "Strategy"):
    """Create simplified performance report with equity curve only."""
    print(f"\nGenerating performance report for {strategy_name}...")
    
    # Only create equity curve and drawdown chart
    plot_equity_curve(results, title=f"{strategy_name} - Equity Curve", save=True)
    
    print(f"Equity curve saved to {RESULTS_DIR}")

# Keep all the existing plotting functions but only use equity curve in the main report
# The other functions (plot_returns_distribution, plot_trade_analysis, plot_monthly_returns) 
# remain available for future use but aren't called in the main report